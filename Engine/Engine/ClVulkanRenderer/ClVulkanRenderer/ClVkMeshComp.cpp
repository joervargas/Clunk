...
namespace Clunk
{
    ...
    void ClVkMeshComp::LoadFromFile(String MeshPath, String TexturePath, Vec3 Color)
    {
        ...
        if(!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, MeshPath.c_str()))
        {
            ...
            THROW_EXCEPTION("Failed to load Mesh!: %s, %s", MeshPath.c_str(), err_string.c_str());
        }
        ...
        m_VertBuffer.Create<Vertex3d>(*cmd_pools->GetGraphicsPool(), *vkctx->QueueFamilies->Graphics(), vertices, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
        m_IndexBuffer.Create<u32>(*cmd_pools->GetGraphicsPool(), *vkctx->QueueFamilies->Graphics(), indices, VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
        m_Texture.FromFile(TexturePath.c_str());
    }

    void ClVkMeshComp::Unload()
    {
        m_Texture.Destroy();
        ...
        )
    {
        ...
        vkCmdPushConstants(DrawBuffer, PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(Transform3d), &MeshTransform);
