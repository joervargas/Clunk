...
namespace Clunk::Vk
{
    ClVkSimple2dLayer::ClVkSimple2dLayer(ClVkContext& VkCtx)
    {
        ...
        // create buffer
        VkDeviceSize buffer_size = sizeof(Simple2dVertex) * VERTICES_DATA.size();
        ClVkBuffer staging_triangle_vert = cl_create_vk_buffer(VkCtx, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT, buffer_size);

        void* data_ptr = nullptr;
        vmaMapMemory(VkCtx.MemAllocator, staging_triangle_vert.Allocation, &data_ptr);
            memcpy(data_ptr, VERTICES_DATA.data(), (size_t)buffer_size);
        vmaUnmapMemory(VkCtx.MemAllocator, staging_triangle_vert.Allocation);

        mVerts = cl_create_vk_buffer(VkCtx, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, 0, buffer_size);
        VkCommandBuffer cmd_buffer = cl_begin_single_time_vk_command_buffer(VkCtx);
            copy_vk_buffer(VkCtx.Device, cmd_buffer, staging_triangle_vert.Handle, mVerts.Handle, buffer_size);
    ...
    {
        // mDescriptor.Pool = cl_create_vk_desc_pool(VkCtx, 0, 0, 0);
        ...
        //     .descriptorSetCount = mDescriptor.Layouts.size(),
    ...
    void ClVkSimple2dLayer::CreatePipeline(const ClVkContext &VkCtx, std::vector<ClVkShaderModule> &ShaderModules, ClVkRenderPass &RenderPass, VkPipelineLayout &Layout, VkExtent2D CustomExtent)
    {
        ...
        std::array<VkVertexInputAttributeDescription, 2>  vert_attributes = Simple2dVertex::GetAttibDesc();
    ...
    void ClVkSimple2dLayer::Draw(const ClVkContext& VkCtx, VkCommandBuffer CmdBuffer)
    {
        ...
        VkBuffer vert_buffers[] = { mVerts.Handle };
        vkCmdBindVertexBuffers(CmdBuffer, 0, 1, vert_buffers, offsets);
